...
export function createGoogleProvider({ fetchImpl = fetch, env = process.env } = {}) {
  ...
  return {
    ...
    async spreadsheetValues(accessToken, spreadsheetId, range) {
      const url = `${sheetsBaseUrl}/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}?valueRenderOption=UNFORMATTED_VALUE&dateTimeRenderOption=SERIAL_NUMBER&majorDimension=ROWS`;
