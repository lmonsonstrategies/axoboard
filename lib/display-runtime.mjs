...
export function createDisplayRuntime({ pool, env = process.env, sendJson, readJson, sameOrigin, isRateLimited, loadWorkspaceDisplay }) {
  ...
  const playerUrl = String(env.AXOBOARD_DISPLAY_BASE_URL || 'https://tv.axoboard.io').replace(/\/$/, '');
  ...
  async function runtime(req, res) {
    ...
    const snapshot = await loadWorkspaceDisplay(device.workspace_id, device.content_mode === 'selected_kpis' ? device.kpi_ids : null);
  ...
  async function handleAdmin(req, res, url, session) {
    if (url.pathname === '/api/axoboard/displays' && req.method === 'GET') return list(res, session);
  ...
  async function handlePublic(req, res, url) {
    try {
      ...
      if (url.pathname === '/api/display/runtime' && req.method === 'GET') return await runtime(req, res);
