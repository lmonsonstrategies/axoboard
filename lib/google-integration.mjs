...
export function createGoogleIntegration({ pool, vault, provider, env = process.env, sendJson, readJson, currentSession, sameOrigin }) {
  ...
  async function previewSelection(connection, body) {
    ...
    if (!sheet) throw new HttpError(422, 'sheet_not_found', 'Choose a worksheet from the selected spreadsheet.');
  ...
  async function handleCallback(req, res, url) {
    ...
    try {
      ...
      if (grantedScopes.length && !grantedScopes.includes('https://www.googleapis.com/auth/spreadsheets.readonly')) {
  ...
  async function runProductRequest(req, res, url, session) {
    try { return await handleProductRequest(req, res, url, session); }
      ...
      if (error instanceof GoogleProviderError) {
        ...
        const message = status === 422 ? 'Spreadsheet was not accessible with this Google account.' : status === 409 ? 'Reconnect Google Sheets to continue.' : 'Google Sheets is temporarily unavailable.';
...
export const googleIntegrationInternals = { calculateKpi, normalizeSpreadsheetId, normalizeRange };
