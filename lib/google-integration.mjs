...
const displayTypes = new Set(['scorecard', 'rep_cards', 'leaderboard', 'table']);
...
function displayPayload(values, aggregation, includeHeaders, displayType, comparisonValues = null, comparisonIncludeHeaders = false) {
  if (displayType === 'scorecard') return null;
  ...
  if (includeHeaders && normalized.length >= 2) {
    labels = normalized[0].map((value, index) => String(value || `Column ${columnLabel(index + 1)}`));
    series = labels.map((_, column) => normalized.slice(1).map((row) => [row[column]]));
    if (comparisonRows.length) {
      const comparisonData = comparisonIncludeHeaders ? comparisonRows.slice(1) : comparisonRows;
      comparisonSeries = labels.map((_, column) => comparisonData.map((row) => [row?.[column] ?? '']));
    }
  } else if (normalized.length >= 1 && width >= 2) {
    labels = normalized.map((row, index) => String(row[0] || `Row ${index + 1}`));
    series = normalized.map((row) => [row.slice(1)]);
    const comparisonData = comparisonIncludeHeaders ? comparisonRows.slice(1) : comparisonRows;
    comparisonSeries = normalized.map((_, index) => [comparisonData[index]?.slice(1) || []]);
  } else {
    throw new HttpError(422, 'display_requires_labels', 'Rep cards and leaderboards need labels plus values. Include a header row or select a label column beside the values.');
  }

  const items = labels.map((label, index) => {
    const calculation = calculateKpi(series[index], aggregation, false);
    let comparisonValue = null;
    if (comparisonSeries[index]?.flat().some((value) => value !== '' && value !== null && value !== undefined)) {
      comparisonValue = calculateKpi(comparisonSeries[index], aggregation, false).value;
    }
    return { label, value: calculation.value, comparisonValue };
  });
...
export function createGoogleIntegration({ pool, vault, provider, env = process.env, sendJson, readJson, currentSession, sameOrigin }) {
  ...
  async function previewSelection(connection, body) {
    ...
    return {
      ...
      displayType, displayPayload: displayPayload(result.body.values, aggregation, includeHeaders, displayType, comparisonValues, body.comparisonIncludeHeaders === true),
