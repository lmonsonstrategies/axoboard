...
export function createGoogleIntegration({ pool, vault, provider, env = process.env, sendJson, readJson, currentSession, sameOrigin }) {
  ...
  async function activationForWorkspace(workspaceId, { includeTiming = false } = {}) {
    const result = await pool.query(`SELECT
      (SELECT MIN(created_at) FROM integration_connections WHERE workspace_id=$1 AND status='healthy') AS connected_at,
      (SELECT MIN(created_at) FROM kpi_mappings WHERE workspace_id=$1 AND status<>'deleted') AS first_kpi_at`, [workspaceId]);
    const connectedAt = result.rows[0]?.connected_at || null;
    const firstKpiAt = result.rows[0]?.first_kpi_at || null;
    const reachedFirstKpi = Boolean(firstKpiAt);
    if (!includeTiming) return { reachedFirstKpi };
    const secondsToFirstKpi = connectedAt && firstKpiAt
      ? Math.floor((new Date(firstKpiAt).getTime() - new Date(connectedAt).getTime()) / 1000)
      : null;
    return {
      connectedAt,
      firstKpiAt,
