...
export function createGoogleIntegration({ pool, vault, provider, env = process.env, sendJson, readJson, currentSession, sameOrigin }) {
  ...
  async function activationForWorkspace(workspaceId, { includeTiming = false } = {}) {
    // Connection rows are created only after a successful OAuth exchange, and both
    // connection disconnects and KPI deletes are tombstones. Their creation times
    // are therefore the immutable lifecycle facts; mutable status must not rewrite
    // historical activation telemetry.
    const result = await pool.query(`SELECT
      (SELECT MIN(created_at) FROM integration_connections WHERE workspace_id=$1) AS connected_at,
      (SELECT MIN(created_at) FROM kpi_mappings WHERE workspace_id=$1) AS first_kpi_at,
      NOW() AS database_now`, [workspaceId]);
    const row = result.rows[0] || {};
    const reachedFirstKpi = Boolean(row.first_kpi_at);
    if (!includeTiming) return { reachedFirstKpi };
    const databaseNowMs = new Date(row.database_now).getTime();
    const connectedAtMs = row.connected_at ? new Date(row.connected_at).getTime() : null;
    const firstKpiAtMs = row.first_kpi_at ? new Date(row.first_kpi_at).getTime() : null;
    const connectedAt = Number.isFinite(connectedAtMs) && connectedAtMs <= databaseNowMs ? row.connected_at : null;
    const firstKpiAt = Number.isFinite(firstKpiAtMs) && firstKpiAtMs <= databaseNowMs
      && connectedAt && firstKpiAtMs >= connectedAtMs ? row.first_kpi_at : null;
    const secondsToFirstKpi = connectedAt && firstKpiAt
      ? Math.floor((firstKpiAtMs - connectedAtMs) / 1000)
      : null;
    return {
      connectedAt,
      firstKpiAt,
      secondsToFirstKpi,
      reachedFirstKpi,
      underTenMinutes: secondsToFirstKpi === null ? false : secondsToFirstKpi >= 0 && secondsToFirstKpi < 600
    };
  }
  ...
  async function bootstrap(res, session) {
    ...
    const [connections, kpis, stored, brand, events, activation] = await Promise.all([
      ...
      activationForWorkspace(session.workspace_id, { includeTiming: canAdminister(session) })
    ]);
