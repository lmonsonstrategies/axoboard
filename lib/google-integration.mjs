...
export function createGoogleIntegration({ pool, vault, provider, env = process.env, sendJson, readJson, currentSession, sameOrigin }) {
  ...
  function publicKpi(row) {
    return {
      ...
      certification: row.engagement?.certification || null,
  ...
  async function kpisForWorkspace(workspaceId) {
    ...
    return result.rows.map((row) => publicKpi({ ...row, engagement: engagement.get(row.id) || null }));
  }
  ...
  async function bootstrap(res, session) {
    ...
    const [connections, kpis, stored, brand, events] = await Promise.all([
      ...
      engagementEnabled ? listDomainEvents(pool, session.workspace_id, 50) : Promise.resolve([])
    ]);
  ...
  async function saveKpi(req, res, session) {
    ...
    try {
      ...
      if (engagementEnabled) await recordSnapshotEngagement(client, { mapping: engagementMapping, snapshotId, value: preview.value, goalValue: preview.goalValue ?? goalValue, fetchedAt: preview.fetchedAt });
    ...
    return sendJson(res, 201, { kpi: { id: mappingId, name, value: preview.value, goalValue: preview.goalValue ?? goalValue, goalSource: preview.goalSource, comparison: preview.comparison, status: 'active', sourceRange: preview.sourceRange, fetchedAt: preview.fetchedAt, lineageHash } });
  }
  ...
  async function updateKpi(req, res, session, mappingId) {
    ...
    return sendJson(res, 200, { kpi: { id, name, value: preview.value, goalValue: preview.goalValue ?? goalValue, goalSource: preview.goalSource, status: 'active', sourceRange: preview.sourceRange, fetchedAt: preview.fetchedAt, lineageHash } });
  }
  ...
  async function performSync(mapping, connection) {
    ...
    try {
      ...
      try {
        ...
        if (engagementEnabled) await recordSnapshotEngagement(client, { mapping: engagementMapping, snapshotId, value: preview.value, goalValue: preview.goalValue ?? mapping.goal_value, fetchedAt: preview.fetchedAt });
  ...
  async function handleProductRequest(req, res, url, session) {
    ...
    if (trustMatch && req.method === 'GET') {
      ...
      return sendJson(res, 200, { metric: trust });
    }
