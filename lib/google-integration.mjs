...
function displayPayload(values, aggregation, includeHeaders, displayType, comparisonValues = null, comparisonIncludeHeaders = false) {
  ...
  if (!includeHeaders) {
    throw new HttpError(422, 'display_requires_headers', 'Turn on “Use first row as headers” for rep cards and leaderboards.');
  }
...
function summarizeDisplay(values, includeHeaders, displayType, payload) {
  ...
  if (displayType === 'scorecard') return calculateKpi(values, 'single_value', includeHeaders);
...
export function createGoogleIntegration({ pool, vault, provider, env = process.env, sendJson, readJson, currentSession, sameOrigin }) {
  ...
  async function previewSelection(connection, body) {
    ...
    const aggregation = displayType === 'scorecard' ? 'single_value' : 'sum';
    ...
    if (comparisonRangeInput) {
      ...
      const comparisonAggregation = displayType === 'scorecard' ? 'single_value' : 'sum';
    ...
    if (comparisonContext) {
      const comparisonCalculation = displayType === 'scorecard'
