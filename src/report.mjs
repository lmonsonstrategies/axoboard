...
function htmlDocument(report) {
  const rows = report.results.map((result) => {
    ...
    return `<tr><td>${escapeHtml(result.routeId)}</td><td>${escapeHtml(result.state)}</td><td>${escapeHtml(result.theme)}</td><td>${escapeHtml(result.viewport.id)}</td><td>${result.status ?? '—'}</td><td>${counts.P0}</td><td>${counts.P1}</td><td>${counts.P2}</td><td>${escapeHtml(result.rubric.score.toFixed(1))}</td><td>${escapeHtml(result.rubric.expertReviewStatus)}</td><td>${screenshot}</td></tr>`;
  }).join('');
  const findingRows = report.findings.map((finding) => `<tr><td><b>${escapeHtml(finding.severity)}</b></td><td>${escapeHtml(finding.rule)}</td><td>${escapeHtml(finding.route)} / ${escapeHtml(finding.state)} / ${escapeHtml(finding.theme)} / ${escapeHtml(finding.viewport)}</td><td>${escapeHtml(finding.message)}</td></tr>`).join('');
  return `<!doctype html>
