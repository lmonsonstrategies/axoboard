...
- Google Sheets has a server-backed vertical slice: one-time PKCE OAuth, encrypted refresh tokens, spreadsheet/worksheet/range selection, persisted KPI snapshots, scheduled refresh, visible lineage, and disconnect/revoke.
...
Create an AxoBoard-owned Google OAuth web application, enable the Google Sheets API, and register this exact callback:

```text
https://axoboard.io/api/integrations/oauth/google/callback
```

Set `AXOBOARD_GOOGLE_CLIENT_ID`, `AXOBOARD_GOOGLE_CLIENT_SECRET`, `AXOBOARD_GOOGLE_REDIRECT_URI`, and a new base64-encoded 32-byte `AXOBOARD_OAUTH_ENCRYPTION_KEY` in Railway. The first slice requests `openid`, `email`, and `spreadsheets.readonly`; it does not request Drive-wide access. `AXOBOARD_SYNC_INTERVAL_MS` controls how often the worker claims due mappings, while each mapping defaults to a five-minute refresh.
