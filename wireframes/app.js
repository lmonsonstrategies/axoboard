...
const defaultBetaState = { activeWorkspace: 'murphy-example', workspaceName: 'Murphy Door', brandColor: '#E96F98', celebrationLanguage: 'Big win!', teamOne: 'Team One', teamTwo: 'Team Two', murphyDemoData: false, completedWorkflows: [], draftCount: 0, lastSavedAt: null };
...
function insertWorkspaceSandboxStates() {
  const states = {
    ...
    workspace: '<span class="empty-axo">M</span><small>MURPHY DOOR · EXAMPLE CUSTOMER</small><h2>Clean onboarding state</h2><p>0 members · 0 connections · 0 dashboards · 0 displays. This tenant intentionally starts empty for repeatable signup testing.</p><div><button class="button button-primary" type="button" data-screen="integrations">Begin with an integration</button><button class="button button-ghost" type="button" data-reset-murphy>Reset to blank</button></div>',
  ...
  ['kombat', 'brand'].forEach((route) => {
    const panel = document.querySelector(`[data-screen-panel="${route}"]`);
    panel.querySelector('.page-header').insertAdjacentHTML('afterend', '<aside class="surface blank-tenant-note"><b>Murphy Door example customer</b><span>This starts as an unpublished, tenant-local draft with no inherited assets or settings.</span><button type="button" data-reset-murphy>Reset workspace</button></aside>');
  });
  document.querySelector('[data-screen-panel="dashboard"] .page-header').insertAdjacentHTML('afterend', '<aside class="surface synthetic-data-banner"><b>SYNTHETIC TEST DATA</b><span>These KPI values are fixtures for visibility testing—not live Murphy Door, Google, or HubSpot data.</span><button type="button" data-reset-murphy>Remove demo data</button></aside>');
}

function applyWorkspaceMode() {
  const id = workspaceProfiles[betaState.activeWorkspace] ? betaState.activeWorkspace : 'murphy-example';
  const profile = workspaceProfiles[id];
  const isMurphy = id === 'murphy-example';
  document.body.dataset.activeWorkspace = id;
  document.body.dataset.demoData = isMurphy && betaState.murphyDemoData ? 'true' : 'false';
  ...
  teamOneInput.value = isMurphy ? 'Team One' : betaState.teamOne || profile.teamOne;
  teamTwoInput.value = isMurphy ? 'Team Two' : betaState.teamTwo || profile.teamTwo;
  document.querySelector('#dashboardTitle').textContent = isMurphy ? 'Murphy Door test dashboard' : 'Sales command pond';
  syncGamePreview();
  syncBrandPreview();
}

function resetMurphyWorkspace() {
  oauthAttempt = 0;
  activeOauthProvider = null;
  persistBetaState({ activeWorkspace: 'murphy-example', workspaceName: 'Murphy Door', murphyDemoData: false, teamOne: 'Team One', teamTwo: 'Team Two' });
  applyWorkspaceMode();
  showToast('Murphy Door reset', '0 KPIs, connections, members, screens, rules, sounds, and stored OAuth test sessions.');
}
...
const workflowDefinitions = {
  workspace: { eyebrow: 'WORKSPACE ACCESS', title: 'Switch workspace', description: 'Move between tenants without mixing data, brands, permissions, or credentials.', steps: ['Choose workspace','Confirm context'], primary: 'Switch workspace', canvas: '<h3>Your workspaces</h3><p>Murphy Door is a blank example customer; Acme Sales retains populated prototype fixtures.</p><div class="workflow-grid"><button class="workflow-card" data-workspace-id="murphy-example" type="button"><span>M</span><strong>Murphy Door</strong><small>Owner · blank OAuth test workspace</small><i>Choose</i></button><button class="workflow-card" data-workspace-id="acme" type="button"><span>A</span><strong>Acme Sales</strong><small>Owner · populated sample tenant</small><i>Choose</i></button></div>', context: '<h3>Tenant boundary</h3><p>Dashboards, integrations, assets, rules, tokens, and games remain isolated by workspace.</p><div class="workflow-summary"><div><small>MURPHY DOOR</small><strong>No inherited credentials</strong></div><div><small>SESSION</small><strong>Revalidated on switch</strong></div></div>' },
  ...
  screen: { eyebrow: 'DISPLAY CONTROL', title: 'Manage TV screen', description: 'Pair, assign content, set a schedule, and diagnose the player remotely.', steps: ['Screen','Content','Schedule','Confirm'], primary: 'Apply to screen', canvas: '<h3>Screen and content</h3><p>Updates apply on the next player heartbeat.</p><div class="workflow-form"><div class="field-row"><label>Screen name<input value="Sales Floor TV" /></label><label>Location<input value="Front office" /></label></div><label>Content<select><option>Revenue pulse · 3 views</option><option>Sales performance</option><option>Murphy Kombat</option><option>Concierge Pulse</option></select></label><div class="field-row"><label>Wake<select><option>7:00 AM</option><option>Always on</option></select></label><label>Sleep<select><option>8:00 PM</option><option>Never</option></select></label></div><div class="workflow-checks"><label><input type="checkbox" checked /> Auto-recover last-known-good content</label><label><input type="checkbox" /> Notify admin when offline for 5 minutes</label></div></div>', context: '<h3>Player heartbeat</h3><p>Chrome · 4K · player v0.4.1</p><div class="workflow-summary"><div><small>STATUS</small><strong>Online · 18s ago</strong></div><div><small>LAST RENDER</small><strong>Successful</strong></div><div><small>PAIRING</small><strong>Device-bound token</strong></div></div>' },
  ...
  billing: { eyebrow: 'PLAN & BILLING', title: 'Manage plan and usage', description: 'Understand limits, billing ownership, invoices, and what grows with the customer.', steps: ['Plan','Usage','Billing','Confirm'], primary: 'Review plan', canvas: '<h3>Commercial plan</h3><p>Pricing remains configurable while the design-partner offer is validated.</p><div class="workflow-grid"><button class="workflow-card" type="button"><span>◌</span><strong>Starter</strong><small>Small teams · core dashboards</small><i>Define before launch</i></button><button class="workflow-card is-selected" type="button"><span>✦</span><strong>Growth beta</strong><small>Displays, automations, sounds, and games</small><i>Current plan</i></button><button class="workflow-card" type="button"><span>◇</span><strong>Scale</strong><small>SSO, governance, support, higher limits</small><i>Define before launch</i></button><button class="workflow-card" type="button"><span>＋</span><strong>Custom</strong><small>Contracted limits and onboarding</small><i>Contact sales</i></button></div>', context: '<h3>Metered dimensions</h3><p>Keep pricing easy to explain and hard to surprise customers with.</p><div class="workflow-summary"><div><small>PRIMARY METER</small><strong>Active workspace</strong></div><div><small>CAPACITY</small><strong>Members · screens · sources</strong></div><div><small>OVERAGE POLICY</small><strong>Warn before charge</strong></div></div>' },
  ...
  oauth: { eyebrow: 'FRESH OAUTH TEST', title: 'Authorize a new connection', description: 'Start from provider consent without importing or reusing any existing Murphy Door credential.', steps: ['Preflight','Consent','Callback','Verify'], primary: 'Review fresh consent', canvas: '<h3 data-oauth-heading>Fresh OAuth preflight</h3><p data-oauth-copy>No cached account or connection will be used.</p><div class="oauth-contract"><div><small>ATTEMPT</small><strong data-oauth-attempt>New session</strong></div><div><small>TENANT</small><strong>Murphy Door example only</strong></div><div><small>CREDENTIAL REUSE</small><strong>Blocked</strong></div><div><small>LIVE TOKEN</small><strong>Not stored in this prototype</strong></div></div><div class="workflow-checks"><label><input type="checkbox" checked disabled /> Generate a unique state value for callback validation</label><label><input type="checkbox" checked disabled /> Use AxoBoard-owned client credentials only</label><label><input type="checkbox" checked disabled /> Encrypt tenant-scoped tokens server-side in production</label><label><input type="checkbox" checked disabled /> Expose disconnect, revocation, expiry, and retry status</label></div>', context: '<h3>Prototype boundary</h3><p data-oauth-boundary>This validates the onboarding contract, not a real provider token exchange.</p><div class="credential-boundary compact"><b>No credential import</b><span>The existing Murphy Google service account, HubSpot portal tokens, browser sessions, and prior OAuth grants are not read or copied.</span></div><div class="workflow-summary"><div><small>CALLBACK</small><strong>/api/oauth/{provider}/callback</strong></div><div><small>STATUS</small><strong>Provider app registration required</strong></div></div>' },
...
document.querySelector('#workflowPrimary').addEventListener('click', () => {
  ...
  if (selectedWorkspace) {
    ...
    persistBetaState({ activeWorkspace: selectedWorkspace, workspaceName: profile.name, ...(selectedWorkspace === 'murphy-example' ? { teamOne: 'Team One', teamTwo: 'Team Two' } : {}) });
    applyWorkspaceMode();
  }
...
document.querySelectorAll('[data-load-demo]').forEach((button) => button.addEventListener('click', () => {
  persistBetaState({ murphyDemoData: true });
  applyWorkspaceMode();
  showToast('Synthetic KPIs loaded', 'Visibility fixtures only—no Google, HubSpot, or Murphy Door data was accessed.');
}));
document.querySelectorAll('[data-reset-murphy]').forEach((button) => button.addEventListener('click', resetMurphyWorkspace));
...
const workflowBindings = [
  ...
  ['#managePlanButton', 'billing'], ['#supportButton', 'support'], ['#setupGuideButton', 'guide']
];
